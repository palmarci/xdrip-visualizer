...
def create_graphs(data, last_days, min_acceptable_bg, max_acceptable_bg, no_widescreen):
	...
	response = requests.get(plotly_js_url)

	if response.status_code != 200:
		raise Exception("Could not download and bake in plotly js library.")
	else:
		print("downloaded latest plotly library")
	
	plotly_js_content = response.text
	...
	html_data += f"<div>Average Blood Glucose: {avg_bg} mmol/l for the last {last_days} days<br>"
	html_data += f"Generated on {datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')}</div><br><br>"
	...
	for date_str in sorted(data_by_date.keys(), reverse=True):  
		...
		trace = ply.Scatter(name="Bloog Sugar", x=measurements['timestamps'], y=measurements['values'],
							mode='lines', marker={'color': colors}, line={'color': 'black'}, showlegend=False)

		
		line1 = ply.Scatter(x=measurements['timestamps'], y=[min_acceptable_bg] * len(measurements['timestamps']),
						   mode='lines', line={'color': 'red', "dash":"dot"}, name='Min', showlegend=False)
		line2 = ply.Scatter(x=measurements['timestamps'], y=[max_acceptable_bg] * len(measurements['timestamps']),
						   mode='lines', line={'color': 'red',"dash":"dot"}, name=f'Max', showlegend=False)

		fig = ply.Figure(data=[trace, line1, line2])
		...
		html_data += f"<h2>Date: {date_str}</h2>"
	...
	html_filename = 'xdrip_database.html'
	pdf_filename = html_filename.replace(".html", ".pdf")

	print(f'writing html file to {html_filename}')
	...
	while i < len(treatments) - 1:
		...
		if isinstance(current_treatment, Insulin) and isinstance(next_treatment, Insulin):
			...
			if time_difference <= min_difference:
				current_treatment.timestamp -= datetime.timedelta(minutes=min_difference/2)
				next_treatment.timestamp += datetime.timedelta(minutes=min_difference/2)
...
if __name__ == "__main__":
	...
	default_acc_min = 4
	default_acc_max = 10
	parser.add_argument("--last-days", type=int, default=7, help=f"Number of last days to create graphs for (default: {default_days})")
	parser.add_argument("--min-long-insulin", type=int, default=15, help=f"Minimum value for long insulin (default: {default_long_min})")
	parser.add_argument("--max-long-insulin", type=int, default=25, help=f"Maximum value for long insulin (default: {default_long_max})")
	parser.add_argument("--min-acceptable-bg", type=float, default=4, help=f"Minimum acceptable blood sugar value (default: {default_acc_min})")
	parser.add_argument("--max-acceptable-bg", type=float, default=10, help=f"Maximum acceptable blood sugar value (default: {default_acc_max})")
